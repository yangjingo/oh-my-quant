...
export const ERRORS = {
  ...
  DATA_BACKEND_UNAVAILABLE: {
    ...
    problem: "Data backends are not ready",
    cause: "Required API keys or local data backend configuration is missing",
    fix: "Check API keys in /config and verify local data backend configuration.",
    docs: "/config",
  },
  DATA_REQUEST_FAILED: {
  ...
  INPUT_INVALID_SYMBOL: {
    ...
    fix: "A-shares: use format like 000001.SZ or 600519.SH. US: use ticker like AAPL. Configure the source in /config and let the agent fetch the symbol.",
    docs: "/config",
  },
  INPUT_MISSING_ARG: {

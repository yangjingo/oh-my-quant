...
async function findExecutableOnPath(name: string): Promise<string | null> {
	const result =
		process.platform === "win32"
			? await runCommand("where", [name], 5000)
			: await runCommand("which", [name], 5000);
	if (result.status !== 0 || !result.stdout) return null;
	const firstMatch = result.stdout.trim().split(/\r?\n/)[0];
	return firstMatch && (await pathExists(firstMatch)) ? firstMatch : null;
}

function isPowerShellPath(path: string): boolean {
	return /(^|[\\/])(pwsh|powershell)(\.exe)?$/i.test(path);
}
...
function powerShellPrelude(): string {
	return [
		"try { [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false) } catch { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 }",
		"$OutputEncoding = [Console]::OutputEncoding",
		"$ProgressPreference = 'SilentlyContinue'",
		"if ($PSVersionTable.PSVersion.Major -ge 7 -and $null -ne $PSStyle) { $PSStyle.OutputRendering = 'PlainText' }",
...
async function getShellConfig(
	...
	if (customShellPath) {
		if (await pathExists(customShellPath)) {
			if (isPowerShellPath(customShellPath)) {
				return ok({ shell: customShellPath, args: powerShellArgs() });
			}
	...
	if (process.platform === "win32") {
		const requestedShell = process.env.WHYJ_SHELL?.trim().toLowerCase();
		if (requestedShell === "bash" || requestedShell === "git-bash") {
			const bashOnPath = await findBashOnPath();
			if (bashOnPath) return ok({ shell: bashOnPath, args: ["-c"] });
		}
		for (const name of ["pwsh.exe", "powershell.exe"]) {
			const shell = await findExecutableOnPath(name);
			if (shell) return ok({ shell, args: powerShellArgs() });
		}
		...
		return err(new ExecutionError("shell_unavailable", "No PowerShell or bash shell found"));
	}
...
export class NodeExecutionEnv implements ExecutionEnv {
	...
	async exec(
		...
		const isPowerShell = isPowerShellPath(shellConfig.value.shell);
		const commandToRun = isPowerShell ? `${powerShellPrelude()}\n${command}` : command;
		...
		return await new Promise((resolvePromise) => {
			...
			try {
				child = spawn(shellConfig.value.shell, [...shellConfig.value.args, commandToRun], {
					cwd,
					detached: process.platform !== "win32",
					env: getShellEnv(this.shellEnv, {
						NO_COLOR: "1",

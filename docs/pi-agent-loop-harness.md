...
7. 只把 core `AgentEvent` 转发给 app-runtime。
...
TUI 应该从 app-runtime 拿 `UIMessage[]` 和 activity 状态，不应该直接读 pi session tree。原因：
...
AppRuntime.submit(text)
  → dispatchUserMessage(agent, injectContext(text))
